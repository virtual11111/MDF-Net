...
from models.MDF import newfusionEMAU_Net3
from models.threelayer import fusionEMAU_Net3
import matplotlib
...
class Test():
    ...
    def save_segmentation_result(self):
        ...
        self.save_img_path = join(self.path_experiment, 'result_img')  # 修改路径
        os.makedirs(self.save_img_path, exist_ok=True)  # 确保路径存在
    ...
    def save_segmentation_result(self):
        ...
        for i in range(self.test_imgs.shape[0]):
            total_img = concat_result(self.test_imgs[i], self.pred_imgs[i], self.test_masks[i])
            full_path = join(self.save_img_path, "Result_" + img_name_list[i] + '.png')
            dir_name = os.path.dirname(full_path)  # 获取文件路径中的文件夹部分
            os.makedirs(dir_name, exist_ok=True)  # 创建文件夹，如果已存在则不报错
            save_img(total_img, full_path)
...
if __name__ == '__main__':
    ...
    # net = models.Dense_Unet(1, 2).to(device)
    # net = fusionEMAU_Net3(img_ch=args.in_channels, output_ch=args.classes).to(device)
    # net = newfusionEMAU_Net3(img_ch=args.in_channels, output_ch=args.classes).to(device)
