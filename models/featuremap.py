...
class FeatureMap(nn.Module):
    ...
    def forward(self, x, y, visualize_softmax=False):
        ...
        if visualize_softmax:
            # 只取第一个样本和第一个通道的特征图进行可视化
            softmax_vis = result_softmax[0, 0].cpu().detach().numpy()
            plt.imshow(softmax_vis, cmap='viridis')
            plt.colorbar()
